...
#  ╚╗╔╝║ ║   ║║║║╠╣ ║
#   ╚╝ ╩ ╩   ╚╩╝╩╚  ╩
#  VIT Campus WiFi Auto-Login CLI  v2.2
# ============================================================================
...
KEYCHAIN_SERVICE="vit-wifi-autologin"
KEYCHAIN_ACCOUNT_USER="wifi-username"
KEYCHAIN_ACCOUNT_PASS="wifi-password"
CHECK_INTERVAL=60
TARGET_SSID="T-VIT"
LOG_FILE="$HOME/.vit-wifi-autologin.log"
DATA_FILE="$HOME/.vit-wifi-data-usage.dat"
LAUNCHAGENT_LABEL="com.user.vit-wifi-autologin"
LAUNCHAGENT_PLIST="$HOME/Library/LaunchAgents/${LAUNCHAGENT_LABEL}.plist"
MAX_LOG_LINES=1000
VERSION="2.2.0"
SP_CACHE_FILE="/tmp/.vitwifi-sp-cache"
...
notify() { osascript -e "display notification \"$1\" with title \"VIT WiFi\"" 2>/dev/null & }
...
get_credential() { security find-generic-password -s "$KEYCHAIN_SERVICE" -a "$1" -w 2>/dev/null; }
has_credentials() { get_credential "$KEYCHAIN_ACCOUNT_USER" >/dev/null 2>&1 && get_credential "$KEYCHAIN_ACCOUNT_PASS" >/dev/null 2>&1; }

# ── Fast WiFi Info (ioreg + ipconfig — ~0.01s total) ────────────────────────
get_ssid_fast() {
    # ioreg is reliable and fast (~0.01s with -r)
    local ssid
    ssid=$(ioreg -l -n AirPortDriver 2>/dev/null | awk -F'"' '/IO80211SSID" =/{print $4; exit}')
    echo "$ssid"
}
...
get_gateway() { netstat -rn 2>/dev/null | awk '/default.*en/{print $2; exit}'; }
get_dns_servers() { scutil --dns 2>/dev/null | awk '/nameserver\[/{print $3}' | sort -u | head -3 | tr '\n' ' '; }

get_mac_address() {
    local iface; iface=$(get_wifi_interface)
    [[ -n "$iface" ]] && ifconfig "$iface" 2>/dev/null | awk '/ether/{print $2}'
}

# ── Detailed WiFi Info (system_profiler — cached, ~3s first call) ───────────
_sp_data=""

_load_sp_data() {
    # Use file cache if fresh (< 30s old)
    if [[ -f "$SP_CACHE_FILE" ]]; then
        local age
        age=$(( $(date +%s) - $(stat -f%m "$SP_CACHE_FILE" 2>/dev/null || echo 0) ))
        if (( age < 30 )); then
            _sp_data=$(cat "$SP_CACHE_FILE")
            return
        fi
    fi
...
_sp_current_field() {
    # Extract a field value from system_profiler current network block
    # Strips the label (everything up to and including ": ")
    echo "$_sp_data" | awk -v pat="$1" '
        /Current Network Information:/ { found=1; next }
        found && $0 ~ pat { sub(/.*: /, ""); print; exit }
    '
}
...
get_signal_noise() {
    echo "$_sp_data" | awk '
        ...
        f && /Signal \/ Noise:/ { gsub(/^[ \t]+Signal \/ Noise: /,""); print; exit }
    '
}

get_rssi()     { local sn; sn=$(get_signal_noise); echo "$sn" | awk -F'/' '{gsub(/[^0-9-]/,"",$1); print $1}'; }
get_noise()    { local sn; sn=$(get_signal_noise); echo "$sn" | awk -F'/' '{gsub(/[^0-9-]/,"",$2); print $2}'; }
get_channel()  { _sp_current_field "Channel:"; }
get_phy_mode() { _sp_current_field "PHY Mode:"; }
get_tx_rate()  { _sp_current_field "Transmit Rate:"; }
get_mcs()      { _sp_current_field "MCS Index:"; }
get_security() { _sp_current_field "Security:"; }
...
get_ping_latency() {
    local r; r=$(ping -c 1 -t 2 8.8.8.8 2>/dev/null | awk -F'/' '/avg/{print $5}')
    [[ -n "$r" ]] && printf "%.1f ms" "$r" || echo "Timeout"
}
...
signal_bar_graph() {
    ...
    printf "${c}"; (( f > 0 )) && printf '%0.s█' $(seq 1 "$f")
    printf "${DIM}"; (( e > 0 )) && printf '%0.s░' $(seq 1 "$e")
    printf "${RST} ${BOLD}%d%%${RST}" "$q"
}

# ── Data Usage (netstat-based) ──────────────────────────────────────────────
get_interface_bytes() {
    # Returns: ibytes obytes
    netstat -I en0 -b 2>/dev/null | awk 'NR==2{print $7, $10}'
}
...
init_data_tracking() {
    ...
    if [[ ! -f "$DATA_FILE" ]]; then
        ...
        if [[ "$saved_today" != "$today" ]]; then
            ...
            sed -i '' "s/^today_start_in=.*/today_start_in=$(echo "$now_bytes" | awk '{print $1}')/" "$DATA_FILE"
            sed -i '' "s/^today_start_out=.*/today_start_out=$(echo "$now_bytes" | awk '{print $2}')/" "$DATA_FILE"
        fi
    fi
}
...
get_data_usage() {
    ...
    # Handle counter wraps (reboot resets netstat counters)
    local sess_in=0 sess_out=0 day_in=0 day_out=0
    if (( now_in >= session_start_in )); then
        sess_in=$(( now_in - session_start_in ))
    else
        sess_in=$now_in  # counter wrapped
    fi
    if (( now_out >= session_start_out )); then
        sess_out=$(( now_out - session_start_out ))
    else
        sess_out=$now_out
    fi
    if (( now_in >= today_start_in )); then
        day_in=$(( now_in - today_start_in ))
    else
        day_in=$now_in
    fi
    if (( now_out >= today_start_out )); then
        day_out=$(( now_out - today_start_out ))
    else
        day_out=$now_out
    fi
...
cmd_speedtest() {
    echo ""
    echo -e "  ${BRAND}${BOLD}VIT WiFi · Speed Test${RST}"
    ...
    ping1=$(ping -c 1 -t 2 8.8.8.8 2>/dev/null | awk -F'/' '/avg/{print $5}') || ping1="0"
    ping2=$(ping -c 1 -t 2 8.8.8.8 2>/dev/null | awk -F'/' '/avg/{print $5}') || ping2="0"
    ping3=$(ping -c 1 -t 2 8.8.8.8 2>/dev/null | awk -F'/' '/avg/{print $5}') || ping3="0"

    avg_ping=$(echo "scale=1; ($ping1 + $ping2 + $ping3) / 3" | bc 2>/dev/null || echo "0")
    # Jitter = max deviation
    ...
    # Multi-size download for better accuracy
    local dl_speed dl_result
    dl_result=$(curl -s -m 15 -o /dev/null \
    ...
    local dl_time=$(echo "$dl_result" | awk '{print $2}')
    local dl_size=$(echo "$dl_result" | awk '{print $3}')
    ...
    # Generate 2MB of data and upload
    local ul_result ul_speed
    ul_result=$(dd if=/dev/urandom bs=1024 count=2048 2>/dev/null | \
        curl -s -m 15 -X POST -o /dev/null \
    ...
    local ul_time=$(echo "$ul_result" | awk '{print $2}')
    local ul_size=$(echo "$ul_result" | awk '{print $3}')

    local ul_formatted
    ul_formatted=$(format_speed "$ul_speed")
    local ul_color="$BGRN"
...
cmd_setup() {
    echo ""
    echo -e "  ${BRAND}${BOLD}VIT WiFi · Credential Setup${RST}"
    ...
    echo -e "  ${DIM}Stored in macOS Keychain (encrypted).${RST}"
    echo ""
    ...
    echo -e "  ${BGRN}${BOLD}✓ Credentials saved to Keychain${RST}"; echo ""
}

cmd_login() {
    echo ""
    echo -e "  ${BRAND}${BOLD}VIT WiFi · Manual Login${RST}"
    echo -e "  ${DIM}$(repeat_char '─' 30)${RST}"
    echo ""
    has_credentials || { echo -e "  ${RED}✗ Run: vitwifi setup${RST}"; echo ""; return 1; }
    local ssid; ssid=$(get_ssid_fast)
...
cmd_status() {
    echo ""
    echo -e "  ${BRAND}${BOLD}VIT WiFi · Network Status${RST}"
    echo -e "  ${DIM}$(repeat_char '─' 30)${RST}"
    echo ""
    ...
    local ssid rssi noise channel tx_rate phy mcs sec local_ip gateway dns mac

    ssid=$(get_ssid_detailed); rssi=$(get_rssi); noise=$(get_noise)
    channel=$(get_channel); tx_rate=$(get_tx_rate); phy=$(get_phy_mode)
    mcs=$(get_mcs); sec=$(get_security)
    ...
    if [[ -n "$ssid" ]]; then
        ...
        [[ -n "$phy" ]]     && printf "  ${DIM}%-14s${RST} ${WHT}%s${RST}\n" "PHY Mode" "$phy"
        [[ -n "$tx_rate" ]] && printf "  ${DIM}%-14s${RST} ${WHT}%s Mbps${RST}\n" "TX Rate" "$tx_rate"
        [[ -n "$mcs" ]]     && printf "  ${DIM}%-14s${RST} ${WHT}%s${RST}\n" "MCS Index" "$mcs"
    ...
    if [[ -f "$LAUNCHAGENT_PLIST" ]] && launchctl list 2>/dev/null | grep -q "$LAUNCHAGENT_LABEL"; then
        printf "  ${DIM}%-14s${RST} ${BGRN}${BOLD}● Running${RST}  ${DIM}(every %ds)${RST}\n" "Daemon" "$CHECK_INTERVAL"
    elif [[ -f "$LAUNCHAGENT_PLIST" ]]; then
...
cmd_dashboard() {
    ...
    while true; do
        ...
        [[ -f "$SP_CACHE_FILE" ]] && _sp_data=$(cat "$SP_CACHE_FILE")
...
H
        ...
        local ssid rssi noise channel tx_rate phy mcs
        ...
        tx_rate=$(get_tx_rate); phy=$(get_phy_mode); mcs=$(get_mcs)
...
cmd_install() {
    echo -e "\n  ${BRAND}${BOLD}VIT WiFi · Install Service${RST}\n  ${DIM}$(repeat_char '─' 30)${RST}\n"
    has_credentials || { echo -e "  ${RED}✗ Run: vitwifi setup${RST}\n"; return 1; }
...
cmd_uninstall() {
    echo -e "\n  ${BRAND}${BOLD}VIT WiFi · Uninstall${RST}\n"
    [[ -f "$LAUNCHAGENT_PLIST" ]] && { launchctl unload "$LAUNCHAGENT_PLIST" 2>/dev/null; rm -f "$LAUNCHAGENT_PLIST"; echo -e "  ${BGRN}✓ Removed.${RST}"; } || echo -e "  ${DIM}Nothing installed.${RST}"
    echo ""
}
...
case "${1:-}" in
    ...
    "")                      cmd_interactive ;;
    *) echo -e "  ${RED}Unknown: $1${RST}  ${DIM}— vitwifi help${RST}"; exit 1 ;;
esac
